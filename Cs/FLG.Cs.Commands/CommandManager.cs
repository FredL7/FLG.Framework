    ...
    public class CommandManager : ICommandManager {
        ...
        public void ExecuteCommand(string commandMessage)
        {
            ...
            try
            {
                ...
                if (fullType == null)
                {
                    // Check one namespace up: i.e. Assembly for datamodel is FLG.Cs.Datamodel, but namespace can be FLG.Cs.Datamodel.Logger
                    lastDot = assemblyString.LastIndexOf('.');
                    assemblyString = type[..lastDot];
                    fullType = Type.GetType($"{type}, {assemblyString}");
                    if (fullType == null)
                    {
